...
var ymd =function (opt) {
    ...
    for (var i = dCurYear; i >= (opt.startYear ? opt.startYear : 1930); i--) {
        if (i == (opt.order ? 1970 : dCurYear)) {
            str = '<li class="custom-selector-item secondListTitle" data-down-role="dataSelect"><span>' + i + '</span></li>';
        } else {
            str = '<li class="custom-selector-item secondListTitle" data-down-role="dataSelect"><span>' + i + '</span></li>';
            ;
        }
    ...
    if (opt.month) {
        for (var i = (opt.startMonth ? opt.startMonth : 1); i <= (opt.endMonth ? opt.endMonth : 12); i++) {
            if (i == (opt.order ? 7 : (dDate.getMonth() + 1))) {
                str = '<li class="custom-selector-item thirdListTitle" data-down-role="dataSelect-month"><span>' + (opt.en ? monthArr[i - 1] : (i < 10 ? "0" + i : i)) + '</span></option>';
            } else {
                str = '<li class="custom-selector-item thirdListTitle" data-down-role="dataSelect-month"><span>' + (opt.en ? monthArr[i - 1] : (i < 10 ? "0" + i : i)) + '</span></option>';
            }
    ...
    function TUpdateCal(opt) {
        var dDate = new Date();
        var daysInMonth = TGetDaysInMonth(opt.year.find(".active").text(), opt.month.find(".active").text());
        var str = "";
        var n = "";
        ...
        opt.day.children().not("h4,.recover").remove();
        if (opt.default1) {
            opt.day.css(opt.css);
            opt.day.append('<option value="0"  selected=true>' + (opt.en ? "select" : "选择") + '</option>');
        }
        opt.day.find(".recover").attr("data-year",opt.year.find(".active").text());
        opt.day.find(".recover").attr("data-month",opt.month.find(".active").text());
        for (var d = ((opt.month.find(".active").text() === "05" && opt.startDay) ? opt.startDay : 1); d <= ((opt.month.find(".active").text() === "10" && opt.endDay) ? 30 : parseInt(daysInMonth)); d++) {
            if (d == (n ? n : (opt.order ? 1 : dDate.getDate()))) {
                str = '<li class="custom-selector-item result-item" data-year="'+opt.year.find(".active").text()+'" data-month="'+opt.month.find(".active").text()+'"><span>' + (d < 10 ? "0" + d : d) + '</span></option>';
            } else {
                str = '<li class="custom-selector-item result-item" data-year="'+opt.year.find(".active").text()+'" data-month="'+opt.month.find(".active").text()+'"><span>' + (d < 10 ? "0" + d : d) + '</span></option>';
            }
