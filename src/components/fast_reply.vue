...
export default {
  ...
  methods: {
    ...
    transformBtn(dir) {
      ...
      if (dir === "prev") {
        if (Math.abs(this.transformIndex.i) > 0) {
          ...
          $(".jump-tabs-wrapper").css(
            "transform",
            "translateX(" + this.transformIndex.num + "px)"
          );
        }
      } else if (dir === "next") {
    ...
    getTotalSize() {
      let total = 0,
        size = this.FastReplyList.length;
      document
        .querySelectorAll(".jump-tabs-wrapper .tabsItem")
    ...
    clickToSendReply(item) {
      ...
      store.commit("setTriageContent", {
        content: item.questionDesc,
        isUpload: item.isUpload
      });
      //   if (parseInt(item.isUpload) !== 0) {
      //     store.commit("setTriageContent", {
      //       content: item.questionDesc,
      //       isUpload: item.isUpload
      //     });
      ...
      //     store.commit(
      //       "setFastReply",
      //       this.$store.state.fastReplyContent + item.questionDesc + "  "
      //     );
      //   }
      //   store.commit(
      //     "setFastReply",
      ...
      this.$emit(
        ...
        this.controllerInput.length === 0
          ? item.questionDesc
          : this.controllerInput + " | " + item.questionDesc
      );
    }
  },
  mounted() {
