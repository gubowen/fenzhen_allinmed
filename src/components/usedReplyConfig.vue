<template>
  <section class="main-masker jump-box-config-box used-reply-config on">
    <section class="jump-box-config">
      ...
      <section class="jump-box-term-box" id="ev-used-reply-config-box">
        ...
        <section class="jump-box-term" v-for="(item,index) in replyList">
          <article class="jump-box-term-title">
            ...
            <figure class="jump-box-term-button" v-if="!fixFlagList[index]">
              <p class="fix" @click.stop="fixFlagList[index]=true;fixContentList[index]=item.replyContent">修改</p>
        ...
        <section class="jump-box-member-config" style="display: block;" v-if="fixFlagList[index]">
          <input type="text" class="jump-box-member-input" placeholder="请输入常用回复" v-model="fixContentList[index]"
