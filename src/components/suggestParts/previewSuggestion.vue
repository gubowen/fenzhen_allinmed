<template>
    <section class="preview-suggestion-box" v-if="$store.state.previewShow">
        ...
        <header class="preview-suggestion-title">
            <button type="button" class="btn-border-small-pvw btn-cancel fl" @click="reviseSuggest"
                    v-if="$store.state.previewType ==1 ">修改
            </button>
            <span>初诊建议</span>
            <button type="button" class="btn-primary-small-pvw fr ev-send-suggestion"
                    @click="submitStatus&&sendSuggest()" v-if="$store.state.previewType ==1">发送
            </button>
        </header>
        <section class="preview-suggestion-inner">
                ...
                <section class="patient-message">
                    ...
                    <figcaption class="patient-message-content">
                        ...
                        <article class="disease-base-msg icon-disease-message">
                            <article>
                                <ul>
                                    <li><span>患病专科：</span><span>{{previewSendData.diagnoseResult.majorName?previewSendData.diagnoseResult.majorName:'暂不确定'}}</span>
                                    </li>
                                    <li v-if="previewSendData.diagnoseResult.illnessName != '' && previewSendData.diagnoseResult.illnessName != '暂不确定'">
                                        <span>所患疾病：</span><span class="caseIllnessName">{{previewSendData.diagnoseResult.illnessName}}</span>
                                    </li>
                                    <li v-if="previewSendData.diagnoseResult.operationName != '' && previewSendData.diagnoseResult.operationName != '暂不确定'">
                                        <span>手术建议：</span><span>{{previewSendData.diagnoseResult.operationName}}</span>
                                    </li>
                                </ul>
                            </article>
                        </article>
            ...
            <section class="preview-suggestion-item result_recommendDoctor"
                     v-if="previewSendData.doctorList && previewSendData.doctorList.length>0">
                <header class="preview-suggestion-item-title">
                    <span>推荐医生</span>
                </header>
                        ...
                        <figcaption class="doctor-message-content">
                            <header class="doctor-message-content-head">
                                ...
                                <figure class="doctor-message-tags">
                                    ...
                                    <span class="tags" style="background: #FFF1D4;color: #EB9E00;"
                                          v-if="item.isFreeTimes != 0">首次问诊免费</span>
                                </figure>
                            </header>
                            ...
                            <article class="doctor-message-goodAt ">擅长：<span v-html="item.illnessNameList + item.operationNameList" class="goodAt"></span>
                            </article>
            ...
            <section class="preview-suggestion-item result_examineTestSuggest"
                     v-if="(previewSendData.examineList&&previewSendData.examineList.length>0 )|| (previewSendData.testList&&previewSendData.testList.length>0)">
                <header class="preview-suggestion-item-title">
                    <span>建议检查/检验</span>
                </header>
                <header class="preview-project-title">
                ...
                <section class="preview-project">
                    <section class="preview-project-list">
                        <article class="preview-project-item examine" v-for="item in previewSendData.examineList">
                            {{item.nodeName || item.checkName}}
                        </article>
                        <article class="preview-project-item test" v-for="item in previewSendData.testList">
                            {{item.inspectionName}}
                        </article>
                    </section>
                </section>
            </section>
            <section class="preview-suggestion-item result_teachingKnowledge"
                     v-if="previewSendData.teachingList && previewSendData.teachingList.length>0">
                <header class="preview-suggestion-item-title">
                    <span>患教知识</span>
                </header>
            ...
            <section class="preview-suggestion-item result_disposeSuggest"
                     v-if="previewSendData.disposeList && previewSendData.disposeList.length>0">
                <header class="preview-suggestion-item-title">
                    <span>处置建议</span>
                </header>
                <header class="preview-project-title">
                ...
                <section class="preview-project">
                    <section class="preview-project-list">
                        <article class="preview-project-item dispose" v-for="item in previewSendData.disposeList">
                            {{item.treatmentName}}
                        </article>
                    </section>
                </section>
            </section>
            <section class="preview-suggestion-item result_operationSuggest"
                     v-if="previewSendData.diagnoseResult.operationName&&previewSendData.diagnoseResult.operationName.length>0">
                <header class="preview-suggestion-item-title">
                    <span>手术建议</span>
                </header>
                <header class="preview-project-title">
                ...
                <section class="preview-project">
                    <section class="preview-project-list">
                        <article class="preview-project-item operation">
                            {{previewSendData.diagnoseResult.operationName}}
                        </article>
                    </section>
                </section>
            </section>
                    ...
                    data: that.previewSendData.diagnoseResult,
                        ...
                        if (data.responseObject.responseStatus) {
                            ...
                            new Promise((resolve, reject) => {
                                ...
                                inquiryResult.consultationState="10";
