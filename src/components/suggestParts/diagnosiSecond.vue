      ...
      <section class="config-suggestion-inner">
        ...
        <section class="config-suggestion-viewers">
          <section class="config-suggestion-item recommendDoctor" v-show="cutNum == 0">
            ...
            <!--全选按钮-->
            <button class="config-suggestion-select icon-select-small" :class="{'active':matchDoc.matchDocState}" v-if="matchDoc.matchDocList.length>0&&docCutNum==0" @click="matchDocAllChecked(matchDoc.matchDocList,'match')"><span>全选</span></button>
            <button class="config-suggestion-select icon-select-small" :class="{'active':allDoc.allDocState}" v-if="allDoc.allDocList.length>0&&docCutNum==1" @click="matchDocAllChecked(allDoc.allDocList,'all')"><span>全选</span></button>
            <!--推荐医生最外层-->
              ...
              <section class="check-project-list">
                <article class="check-project-item" v-for="(oneItem,oneIndex) in teachingKnowledgeList" @click.stop="showNext('FirstIndex',oneIndex)">
                  ...
                  <section class="check-project-second-box" :class="{'active':FirstIndex == oneIndex}" v-if="oneItem.children.length>0">
                    <section class='check-project-second-item check-project-second-title' :class="{'icon-downArrow':twoItem.children.length>0,'selected':SecondIndex == twoIndex}" v-for="(twoItem,twoIndex) in oneItem.children" @click.stop="showNext('SecondIndex',twoIndex)">
                      ...
                      <section class="check-project-second-box" v-if="twoItem.children.length>0" :class="{'active':SecondIndex == twoIndex}">
                        <section class="check-project-second-item select-element" :class="{'icon-downArrow':threeItem.children.length>0,'active':threeItem.isChecked}" v-for="(threeItem,threeIndex) in twoItem.children" @click.stop="selectData(threeItem,1)">
                          <i class="icon-select-big"></i>
                          <span>{{threeItem.knowledgeName}}</span>
                          <i class="icon-detail" @click.stop="knowledgeId=threeItem.knowledgeId;configShow = false;teachingDetailsShow=true"><span>详情</span></i>
                          <section class="check-project-second-box" v-if="threeItem.children.length>0">
                            <section class="check-project-second-item select-element" :class="{'icon-downArrow':fourItem.children.length>0,'active':fourItem.isChecked}" v-for="(fourItem,fourIndex) in threeItem.children" @click.stop="selectData(fourItem,1)">
                              <i class="icon-select-big"></i>
                ...
                <article class="check-project-item" v-for="(oneItem,oneIndex) in disposeSuggestList" @click.stop="showNext('FirstIndex',oneIndex)">
                  ...
                  <section class="check-project-second-box" :class="{'active':FirstIndex == oneIndex}" v-if="oneItem.children.length>0">
                    <section class='check-project-second-item check-project-second-title' :class="{'icon-downArrow':twoItem.children.length>0,'selected':SecondIndex == twoIndex}" v-for="(twoItem,twoIndex) in oneItem.children" @click.stop="showNext('SecondIndex',twoIndex)">
                      ...
                      <section class="check-project-second-box" v-if="twoItem.children.length>0" :class="{'active':SecondIndex == twoIndex}">
                        <section :class="{'icon-downArrow':threeItem.children.length>0,'selected':ThirdIndex == threeIndex,'active':threeItem.isChecked}" class="check-project-second-item select-element" v-for="(threeItem,threeIndex) in twoItem.children" @click.stop="showNext('ThirdIndex',threeIndex)">
                          ...
                          <section class="check-project-second-box" v-if="threeItem.children.length>0" :class="{'active':ThirdIndex == twoIndex}">
                            <section class="check-project-second-item select-element" :class="{'icon-downArrow':fourItem.children.length>0,'selected':FourIndex == fourIndex,'select-disabled':!fourItem.isSelected,'active':fourItem.isChecked}" v-for="(fourItem,fourIndex) in threeItem.children">
