    ...
    <section>
        <section class="config-suggestion-box" v-if="maskerShow" v-show="!$store.state.previewShow">
            ...
            <header class="config-suggestion-title">
                <button class="config-suggestion-back" @click="goBackCheck()"><i
                        class="icon-leftArrow"></i><span>上一步</span></button>
                <article>
                    <h2>自定义建议</h2>
                    <p>可通过下列标签选择建议类型</p>
                </article>
                <button class="jump-box-add-term"
                        :class="{'icon-suggestion-preview-gray':isLight,'forbid':isLight,'icon-suggestion-preview':!isLight}"
                        @click="previewSuggest"><span>预览</span></button>
                <button class="jump-box-add-send" :class="{'forbid':isLight}" @click="sendSuggest"><span>发送</span>
                </button>
            </header>
            <section class="config-suggestion-inner scrollTop">
                <nav class="config-suggestion-tabsBox">
                    <ul class="config-suggestion-tabs">
                        <li class="config-suggestion-tabs-item" :class="{'active':cutNum==index}"
                ...
                <section class="config-suggestion-viewers">
                    <section class="config-suggestion-item recommendDoctor" v-show="cutNum == 0">
                        ...
                        <nav class="mate-and-all">
                            <ul>
                                <li :class="{'on':docCutNum == index}" v-for="(item,index) in docTabList"
                                    @click="docCutNum = index">{{item}}
                                </li>
                            </ul>
                        </nav>
                        <!--搜索医生-->
                        ...
                        <!--全选按钮-->
                        <button class="config-suggestion-select icon-select-small"
                                :class="{'active':matchDoc.matchDocState}"
                                v-if="matchDoc.matchDocList.length>0&&docCutNum==0"
                                @click="matchDocAllChecked(matchDoc.matchDocList,'match')"><span>全选</span></button>
                        <button class="config-suggestion-select icon-select-small"
                                :class="{'active':allDoc.allDocState}" v-if="allDoc.allDocList.length>0&&docCutNum==1"
                        ...
                        <section class="doctor-message-box">
                            <section class="doctor-message doctor-message-mate" v-show="docCutNum==0">
                                <template v-if="matchDoc.matchDocList.length>0">
                                    <section class='doctor-message-item icon-select-big'
                                             :class="{'active':docList.isChecked}"
                                             v-for="(docList,index) in matchDoc.matchDocList"
                                             @click.stop="changeCheckedState(docList,index,'match')">
                                        <figure class="doctor-message-img"><img
                                                :src="docList.logoUrl?docList.logoUrl:'/static/img/img00/common/default_logo.png'">
                                        </figure>
                        ...
                        <h4>{{docList.fullName}}</h4>
                          ...
                          <span class="tags" v-if="docList.isTop==1">全国TOP10骨科医院</span>
                                                    ...
                                                    <span class="tags" v-if="docList.isFreeTimes==1"
                                                          style="background: #FFF1D4;color: #EB9E00;">首次问诊免费</span>
                                                </figure>
                                            </header>
                                            ...
                                            <article class="doctor-message-goodAt">擅长：{{docList.illnessNameList}}
                                            </article>
                            ...
                            <section class="doctor-message doctor-message-all" v-show="docCutNum==1">
                                <section class="doctor-message-page-box">
                                    <section class='doctor-message-item icon-select-big'
                                             :class="{'active':docList.isChecked}"
                                             v-for="(docList,index) in allDoc.allDocList"
                        ...
                        <h4>{{docList.fullName}}</h4>
                          ...
                          <span class="tags" v-if="docList.isTop==1">全国TOP10骨科医院</span>
                                            ...
                                            <article class="doctor-message-goodAt">擅长：<span
                                                    v-html="docList.illnessNameList"></span></article>
                                            <article class="doctor-message-num">
                                                <span class="price">¥{{docList.generalPrice}}/{{docList.generalTimes}}次起</span>
                                                <span class="lastNum">仅剩{{docList.adviceNum}}个名额</span>
                                            </article>
