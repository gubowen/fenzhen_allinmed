                    ...
                    <li v-for="ele in chatHistoryRecordList">
                      <article>
                        <header :class="{'doctor-letter':ele.fromAccount == '1_doctor00001'}"
                                v-if=" ele.msgType.toLowerCase()==='file' || ele.msgType.toLowerCase()==='custom' && JSON.parse(ele.body.substring(1,ele.body.length-1)).type != 'new-health'&& JSON.parse(ele.body.substring(1,ele.body.length-1)).type != 'medicalReport'&& JSON.parse(ele.body.substring(1,ele.body.length-1)).data.actionType!==5">
